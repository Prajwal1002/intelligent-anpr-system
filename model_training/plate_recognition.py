...
if len(sys.argv) > 1:
    ...
    # Use most recent file from UPLOAD_DIR
    if not os.path.exists(UPLOAD_DIR):
        print(f"Uploads folder not found at {UPLOAD_DIR}")
        sys.exit(1)
    try:
        filename = max(os.listdir(UPLOAD_DIR), key=lambda x: os.path.getctime(os.path.join(UPLOAD_DIR, x)))
...
# image_path = os.path.join(UPLOAD_DIR, filename)
# image_path = "backend\uploads\dem4.jpg"
...
# Then proceed with WPOD-NET detection and plotting
# Load WPOD-NET Model
def load_wpod_net(json_path, weights_path):
    try:
        ...
        print(f" Error loading WPOD-NET model: {e}")
        return None
# Preprocess Image
...
json_path = os.path.join(script_dir, "wpod-net.json")
weights_path = os.path.join(script_dir, "wpod-net.h5")

wpod_net = load_wpod_net(json_path, weights_path)

# Run License Plate Detection
test_image_path = max(
    [os.path.join(UPLOAD_DIR, f) for f in os.listdir(UPLOAD_DIR)],
    key=os.path.getctime
)  # Automatically fetches the latest uploaded image
print(f"Using image: {test_image_path}")
vehicle, LpImg, cor = get_plate(test_image_path, wpod_net)

# # Ensure that the image is valid before plotting
# if isinstance(vehicle, np.ndarray) and vehicle.size > 0:
#     plt.imshow(vehicle)
#     plt.axis("off")
#     plt.show()
# else:
#     print("Error: No valid image to display!")
...
if cropped is None:
    print(" Could not load the cropped plate image.")
else:
    import matplotlib.pyplot as plt
...
model_path = os.path.join(os.path.dirname(__file__), "improved_custom_CNN.h5")
...
def show_results(model, char_images):
    import numpy as np
    import cv2
    ...
    if not char_images or not isinstance(char_images, list):
        print("No character images to process.")
        return ""

    for idx, ch in enumerate(char_images):
        try:
            if ch is None:
                print(f"Character image at index {idx} is None.")
                continue

            img_ = cv2.resize(ch, (32, 32), interpolation=cv2.INTER_AREA)
            img = fix_dimension(img_) / 255.0
            ...
            y_ = model.predict(img, verbose=0)
            predicted_idx = np.argmax(y_)
            character = dic.get(predicted_idx, '?')
            output.append(character)

        except Exception as e:
            print(f"Error processing character at index {idx}: {str(e)}")
            continue
...
print(json.dumps({"license_plate": plate_number}))
# output_data = {"license_plate": plate_number}
# with open(r"C:\Users\Acer\Desktop\License Plate Full-Stack\frontend\public\output.json", "w") as f:
